...
int main()
{
    ...
    fo(i, 0, n)
    {
        fo(j, 0, n)
        {
            if (i != j)
            {
                if (a3[i][j] == 0 && a3[j][i] == 0)
                {
                    float n1 = (a[i][j] + a[j][i]) / 2;
                    a3[i][j] = 1;
                    a3[j][i] = 1;
                    a2[i][j] = n1;
                    a2[j][i] = n1;
    ...
    fo(i, 0, n){
        fo(j, 0, n){
            if (i != j)
                cout
            << a2[i][j] << fixed << setprecision(8)
    << " ";
    else cout << a2[i][j] << fixed << setprecision(8)
    << " ";
}
line
}
fo(i, 0, n)
{
    fo(j, 0, n)
    {
        if (i != j)
            cout
                << a1[i][j] << fixed << setprecision(8)
        << " ";
        else cout << a1[i][j] << fixed << setprecision(8)
        << " ";
    }
    line
}
