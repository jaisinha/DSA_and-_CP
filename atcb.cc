...
bool solve()
{
    ...
    if (n == 1 && (v[0] <= k) || (v1[0] <= k))
        return true;
    else if (n == 1 && (v[0] > k) && (v1[0] > k))
        return false;
    for (int i = 0; i < v.size() - 1; i++)
    {
        if (abs(v[i] - v[i + 1]) <= k)
            f = 0;
        else if (abs(v1[i] - v1[i + 1]) <= k)
            f = 0;
        else if (abs(v[i] - v1[i + 1]) <= k)
            f = 0;
        else if (abs(v[i + 1] - v1[i]) <= k)
            f = 0;
        else
            return false;
    }
    cout << abs(v[2] - v[3]) << " " << abs(v1[2] - v1[3]) << " " << abs(v[2] - v1[3]) << " " << abs(v[3] - v1[2]) << endl;
    return true;
}
int main()
{
