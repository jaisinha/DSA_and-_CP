...
int main()
{
    ...
    for (auto &i : v)
        cin >> i;
    int ans = -1, count = 0, f = 0, c1 = 1, c0 = 1;
    if (v.size() == 1 && v[0] == 1)
        cout << "0";
    else
    {
        for (int i = 1; i < v.size(); i++)
        {
            ...
            else if (v[i - 1] == 0 && v[i] == 1 && f1 == 1)
                count++;
            else if (v[i - 1] == 0 && v[i] == 1 && f1 == 0)
                ans = max(ans, count);
            else if (v[i - 1] == 1 && v[i] == 0 && f == 0)
            {
                f = 1;
                count += 2;
            }
            else if (f == 1 && v[i - 1] == 1 && v[i] == 0)
            {
                ans = max(ans, count);
            }
            else if (v[i - 1] == 1)
